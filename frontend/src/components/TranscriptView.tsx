import { User, MessageSquare, Download, Share2, MoreHorizontal, ArrowLeft, Clock, Users } from 'lucide-react';
...
export function TranscriptView({ transcript, onBack, speakerMap }: TranscriptViewProps) {
                        ...
                        >
                            ...
                            <div className={cn(
                                ...
                                {segment.context && segment.context.length > 0 && (
                                    <div className={cn(
                                        "absolute top-0 w-64 z-20",
                                        isUser ? "left-full ml-4" : "right-full mr-4"
                                    )}>
                                        {segment.context.map((ctx, ctxIdx) => (
                                            <motion.div
                                                initial={{ opacity: 0, scale: 0.9, y: 10 }}
                                                animate={{ opacity: 1, scale: 1, y: 0 }}
                                                transition={{ delay: 0.2 + (ctxIdx * 0.1) }}
                                                key={ctxIdx}
                                                className="bg-slate-900/90 backdrop-blur-xl border border-indigo-500/30 p-4 rounded-xl shadow-2xl mb-2 text-left"
                                            >
                                                <div className="flex items-center space-x-2 mb-2">
                                                    <div className="w-1.5 h-1.5 bg-indigo-400 rounded-full animate-pulse" />
                                                    <h4 className="text-xs font-bold text-indigo-300 uppercase tracking-wider">{ctx.title}</h4>
                                                </div>
                                                <p className="text-sm text-slate-300 mb-3 leading-snug">{ctx.description}</p>
                                                <a
                                                    href={ctx.action_url}
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center text-[10px] font-bold text-white bg-indigo-600 px-2 py-1 rounded hover:bg-indigo-500 transition-colors"
                                                >
                                                    {ctx.action_text}
                                                </a>
                                            </motion.div>
